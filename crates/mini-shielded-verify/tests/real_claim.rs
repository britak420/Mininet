...
fn a_real_claim_and_its_nullifier_group() -> (Vec<u8>, [u8; 32], Vec<NullifierRecord>) {
    ...
    let spend_blinding = mini_crypto::random_32().unwrap();
    ...
    let group: Vec<NullifierRecord> = claim
        ...
        .collect();
    (claim.encode(), digest, group)
}

#[test]
fn a_genuine_claim_is_approved_end_to_end() {
    let (claim_bytes, digest, group) = a_real_claim_and_its_nullifier_group();
    ...
    assert!(
        verifier.verify_claim(&digest, &group),
...
fn a_tampered_claim_is_rejected_even_though_it_was_stored() {
    let (mut claim_bytes, _digest, _group) = a_real_claim_and_its_nullifier_group();
    ...
    assert!(
        !verifier.verify_claim(&stored_digest, &group),
...
fn wrong_network_id_is_rejected() {
    ...
    assert!(!verifier.verify_claim(&digest, &group));
}
