...
///
/// **Compatibility boundary (Gate #72, F72-01 remediation, D-0509/D-0511):**
/// `blinding_factor` is decoded via [`canonical_scalar`], which rejects the
/// ~15/16 of uniformly random 32-byte strings that are not a canonical
/// encoding below the scalar field order. Before D-0509 this function
/// decoded via `Scalar::from_bytes_mod_order` (silently reducing any
/// 32-byte input), so a blinding factor generated by pre-D-0509 code as raw
/// `mini_crypto::random_32()` bytes (not run through
/// `mini_value::random_scalar_bytes`'s wide-reduction) will, about 15
/// times out of 16, fail to open here even though it opened successfully
/// before this fix. This is safe **only** because this codebase has never
/// had a production deployment or persisted note corpus predating D-0509 —
/// this canonicalization fix and the companion generation-side fix
/// (`random_scalar_bytes`, applied at every blinding-factor call site the
/// same day) shipped together in the same unreleased branch, so no
/// already-encoded note anywhere ever used the old, non-canonical path.
/// **Before this scheme is ever used to persist real, spendable notes**,
/// whoever adds that persistence must either confirm no note created before
/// D-0509 can reach this function, or add an explicit migration step that
/// re-encodes old notes' blinding factors to their canonical reduced form
/// (never by loosening this decoder back to a non-canonical one — that
/// would reopen F72-01).
