...
| **Routing attacks** | A DHT-based discovery layer (`mini-net`, Layer 2 of `ADDRESSING.md`) is vulnerable to classic Kademlia-style attacks: a malicious node can lie about routing tables to isolate or misdirect a peer. | N2 (real transport exists), Layer-1 self-certification (`ADDRESSING.md`), `mini_net::randomized_fanout_peers`/`randomized_dialable_fanout` (D-0473) | Partial — a lying router can only fail to route or misdirect, never forge an identity that passes Layer-1 verification (this is the core claim of `ADDRESSING.md`). D-0473 raises the cost of a *partial* eclipse specifically: an attacker occupying some but not all of a victim's nearest routing positions can no longer guarantee gossip-fanout selection just by proximity, since selection now depends on an unpredictable seed rather than distance. It does nothing against a *fully* eclipsed candidate pool, and bucket refresh by liveness ping (so a full eclipse can't be sustained by simply never being evicted) remains unbuilt — availability-level routing attacks are still not fully defended. |
